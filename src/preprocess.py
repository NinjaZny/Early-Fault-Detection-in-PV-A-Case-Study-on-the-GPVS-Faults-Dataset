...
def _split_indices(n: int, seed: int, ratios):
...
def save_processed_dataset(X: np.ndarray, Y: np.ndarray, label_str: str,
    ...
    tail = f"{CFG.OUTLIER_METHOD}_{CFG.NORMALIZE_METHOD}_{CFG.LOWPASS_FILTER}_{CFG.FEATURE_SELECTION}_Slidingwindow_.npy"
    ...
    # F0 data will be splited into train/val/test dataset
    if label_str.startswith("F0"):
        idx_tr, idx_va, idx_te = _split_indices(X.shape[0], CFG.RANDOM_SEED, CFG.SPLIT_RATIO)
    ...
    # Fault data (F1-F7) will be saved into test dataset
    else:
...
def preprocess_data(df, sensors, outlier_method, normalize_method, lowpass_filter, feature_selection, pca_map=None):    
    ...
    if outlier_method == 'IQR':
        for col in sensors:
            ...
            x_replaced, mask = preprocess_methods.replace_outliers_local_mean(x, window=CFG.OUTLIER_WINDOW)
    ...
    if lowpass_filter == 'butterworth':
        df_filtered = preprocess_methods.butterworth_lowpass_filter_dataframe(df_normalized, sensors, cutoff=CFG.CUTOFF, fs=CFG.FS, order=CFG.BUTTER_ORDER)
    elif lowpass_filter == 'moving_average':
        df_filtered = preprocess_methods.moving_average_filter_dataframe(df_normalized, sensors, window=CFG.MA_WINDOW, center=True, min_periods=1)
